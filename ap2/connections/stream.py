...
class Stream:
    ...
    def __init__(self, stream, buff):
        ...
        if self.type == Stream.REALTIME:
            ...
            self.data_port, self.data_proc, self.audio_connection = AudioRealtime.spawn(self.session_key, self.audio_format, buff, self.session_iv)
        elif self.type == Stream.BUFFERED:
            self.data_port, self.data_proc, self.audio_connection = AudioBuffered.spawn(self.session_key, self.audio_format, buff, iv=None)
